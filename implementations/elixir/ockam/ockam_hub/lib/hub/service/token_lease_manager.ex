defmodule Ockam.TokenLeaseManager do
  ...
  defp initialization(_options) do
    token_manager_opts = Application.get_env(:ockam_hub, :token_manager)
  ...
  defp process_request(received_payload, state) do
    payload = trim_payload_prefix(received_payload)

    response = process_encoded_payload(payload, state)

    length =
      case String.length(response) do
        l when l > 128 and l < 255 ->
          :binary.encode_unsigned(l) <> <<1>>

        l ->
          :binary.encode_unsigned(l)
      end

    length <> response
  end

  defp trim_payload_prefix("{" <> _rest = payload) do
    payload
  end

  defp trim_payload_prefix(<<_new::size(8)>> <> payload) do
    trim_payload_prefix(payload)
  end
